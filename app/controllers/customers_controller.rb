class CustomersController < ApplicationController
  ...
  def show
    ...
    if is_admin?
   @reservation_hist = ReservationHistory.joins("INNER JOIN 'cars' ON cars.id = reservation_histories.car_id").where(:customer_id => params[:id]).select("cars.*, reservation_histories.*").order('created_at DESC')
  end
  end
